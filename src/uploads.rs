...
pub async fn initiate(
    ...
    if !(0..=i64::try_from(config.file_max_bytes).expect("usize fits i64")).contains(&request.size)
    {
    ...
    let (created, upload) = match existing {
        ...
        None => {
            ...
            let inserted = sqlx::query_as::<_, Upload>(
    ...
    response_for_upload(pool, objects, config, upload, created).await
}
...
async fn response_for_upload(
    ...
    if upload.status == "completed" {
        let file = file_for_upload(pool, &upload).await?;
        return Ok((
            false,
            InitiateResponse {
                id: upload.id,
...
pub async fn complete(
    ...
    let upload = owned_upload(pool, auth, id).await?;
    if upload.status == "completed" {
        return Ok((false, file_for_upload(pool, &upload).await?.into()));
    }
    if upload.status == "cleanup_pending" || upload.expires_at <= Utc::now() {
        return Err(ApiError::CodedGone {
            code: "upload_expired",
    ...
    if !claim_completion(pool, id, &lease_token, &candidate_key).await? {
        return Err(ApiError::CodedConflict {
    ...
    let Some(head) = objects
        ...
        restore_pending(pool, id, &lease_token).await?;
        return Err(ApiError::CodedConflict {
    ...
    {
        enqueue_all_versions_pool(pool, &candidate_key).await?;
        restore_pending(pool, id, &lease_token).await?;
        return Err(ApiError::CodedConflict {
    ...
    let file = sqlx::query_as::<_, FileRecord>("INSERT INTO files (id, tenant_id, owner_ref, filename, mime_type, size, storage_key) VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *")
    ...
    let completed = sqlx::query("UPDATE file_uploads SET status = 'completed', completed_at = now(), lease_token = NULL, lease_expires_at = NULL WHERE id = $1 AND status = 'finalizing' AND lease_token = $2 AND expires_at > now() AND lease_expires_at > now()")
        .bind(id)
        .bind(&lease_token)
        .execute(&mut *tx)
        .await?;
    ...
    enqueue_all_versions_after(&mut tx, &upload.staging_key, 305).await?;
    tx.commit().await?;
    Ok((true, file.into()))
}

async fn restore_pending(pool: &PgPool, id: &str, lease_token: &str) -> ApiResult<()> {
    sqlx::query(
...
async fn claim_completion(
    ...
    let mut tx = pool.begin().await?;
    let previous_candidate = sqlx::query_scalar::<_, Option<String>>(
        "SELECT candidate_key FROM file_uploads WHERE id = $1 FOR UPDATE",
    )
    .bind(id)
    ...
    let claimed = sqlx::query(
        "UPDATE file_uploads
         SET status = 'finalizing', lease_token = $2, lease_expires_at = now() + interval '120 seconds', candidate_key = $3
    ...
    if claimed.rows_affected() == 1 {
        if let Some(previous_candidate) = previous_candidate {
            enqueue_all_versions(&mut tx, &previous_candidate).await?;
        }
        tx.commit().await?;
        Ok(true)
    } else {
...
async fn owned_upload(pool: &PgPool, auth: &AuthContext, id: &str) -> ApiResult<Upload> {
    sqlx::query_as::<_, Upload>("SELECT id, tenant_id, owner_ref, request_hash, file_id, filename, mime_type, expected_size, staging_key, status, expires_at FROM file_uploads WHERE id = $1 AND tenant_id = $2 AND owner_ref = $3").bind(id).bind(&auth.tenant_id).bind(&auth.principal_id).fetch_optional(pool).await?.ok_or_else(|| ApiError::NotFound("Upload not found.".into()))
}
async fn file_for_upload(pool: &PgPool, upload: &Upload) -> ApiResult<FileRecord> {
...
pub async fn cleanup_expired(pool: &PgPool, objects: &ObjectStore) -> ApiResult<()> {
    let mut tx = pool.begin().await?;
    let keys = sqlx::query_as::<_, (String, Option<String>)>("UPDATE file_uploads SET status = 'cleanup_pending', lease_token = NULL, lease_expires_at = NULL WHERE expires_at <= now() AND (status = 'pending' OR (status = 'finalizing' AND lease_expires_at <= now())) RETURNING staging_key, candidate_key").fetch_all(&mut *tx).await?;
    for (key, candidate_key) in keys {
        enqueue_all_versions(&mut tx, &key).await?;
        if let Some(candidate_key) = candidate_key {
            enqueue_all_versions(&mut tx, &candidate_key).await?;
        }
    }
    tx.commit().await?;
    let keys = sqlx::query_as::<_, (String, String, Option<String>, bool)>(
        "SELECT id, storage_key, version_id, all_versions FROM object_deletion_outbox WHERE not_before <= now()",
    )
    .fetch_all(pool)
    .await?;
    for (id, key, version_id, all_versions) in keys {
        ...
        if deleted {
            sqlx::query("DELETE FROM object_deletion_outbox WHERE id = $1")
                .bind(id)
                .execute(pool)
                .await?;
        }
...
async fn enqueue_all_versions(
    ...
    sqlx::query("INSERT INTO object_deletion_outbox (id, storage_key, all_versions) VALUES ($1, $2, true) ON CONFLICT DO NOTHING")
...
async fn enqueue_all_versions_after(
    ...
    sqlx::query("INSERT INTO object_deletion_outbox (id, storage_key, all_versions, not_before) VALUES ($1, $2, true, now() + $3 * interval '1 second') ON CONFLICT DO NOTHING")
        .bind(new_id("object_delete")).bind(storage_key).bind(delay_seconds).execute(&mut **tx).await?;
    Ok(())
}

async fn enqueue_all_versions_pool(pool: &PgPool, storage_key: &str) -> ApiResult<()> {
    sqlx::query("INSERT INTO object_deletion_outbox (id, storage_key, all_versions) VALUES ($1, $2, true) ON CONFLICT DO NOTHING")
        .bind(new_id("object_delete")).bind(storage_key).execute(pool).await?;
    Ok(())
}
