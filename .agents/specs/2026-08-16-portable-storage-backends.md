...
Both backends are native, both use `sqlx`, both run under tokio and axum. There
is no wasm target, so no workspace split is required — two traits and two
implementations inside the existing crate are sufficient.

```
src/store/mod.rs        Store trait
src/store/postgres.rs   existing implementation, moved not rewritten
src/store/sqlite.rs     new
src/blob/mod.rs         BlobStore trait
src/blob/s3.rs          existing implementation, moved not rewritten
src/blob/fs.rs          new
```

`src/api.rs`, `src/auth.rs`, `src/capability.rs`, `src/model.rs`, and
`src/error.rs` are already backend-agnostic and are untouched.

### The `Store` trait

Coarse-grained: whole domain operations, not primitives like `begin`, `query`,
or `commit`. Each method is one atomic unit of work, and the implementation is
responsible for delivering that atomicity by whatever mechanism its engine
provides.

```
trait Store {
    async fn start_turn(&self, actor: &Actor, req: StartTurn) -> ApiResult<StartTurnResult>;
    async fn append_items(&self, ...) -> ApiResult<...>;
    async fn complete_turn(&self, ...) -> ApiResult<...>;
    async fn create_conversation(&self, ...) -> ApiResult<...>;
    async fn read_conversation(&self, ...) -> ApiResult<...>;
    async fn list_items(&self, ...) -> ApiResult<...>;
    async fn record_continuation(&self, ...) -> ApiResult<...>;

    async fn begin_upload(&self, ...) -> ApiResult<...>;
    async fn finalize_upload(&self, ...) -> ApiResult<...>;
    async fn delete_file(&self, ...) -> ApiResult<...>;

    async fn drain_deletion_outbox(&self, limit: usize) -> ApiResult<Vec<PendingDelete>>;
    async fn confirm_deletion(&self, ...) -> ApiResult<()>;
}
```
...
Coarse granularity is chosen so that dialect differences stay inside an
implementation instead of leaking into the callers. The cost is that a
storage-touching feature is written twice. That is accepted; the alternative is
a fine-grained trait that leaks `$1`-versus-`?`, `jsonb`-versus-`json1`, and
lock semantics into shared code.
...
- **Two implementations is a permanent tax.** Every storage-touching feature is
  written and tested twice. Justified by the single-node deployment goal, but it
  should be revisited if T2 does not attract real usage.
- **Step 2 is high-risk and delivers nothing visible.** Extracting a trait from
  1357 lines of interleaved domain and SQL logic, with no behaviour change,
  against correctness-critical invariants.
...
1. Should `SQLITE_SYNCHRONOUS` default to `FULL` or `NORMAL`? `FULL` is the safe
   default for a durable ledger; `NORMAL` with WAL is markedly faster and is
   still crash-safe against process death, though not against power loss.
...
3. Does the filesystem backend need a `fsync` on the directory after `rename` to
   guarantee durability of the link itself, and what does that cost per upload?
